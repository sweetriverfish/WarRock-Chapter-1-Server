                ...
                if (Room.State == Enums.RoomState.Waiting)
                {
                    lock (Room._playerLock)
                    {
                        if (Room.Players.All(p => p.Value.Ready))
                        {
                            ...
                            if (debTeam + niuTeam > 1 || Player.User.AccessLevel >= 3 || GameConfig.AllowStartAlone)
                            {
                                if (teamDifference <= GameConfig.MaxTeamDifference || Room.Mode == Enums.Mode.Free_For_All || Player.User.AccessLevel >= 3 || GameConfig.AllowStartAlone)
                                {
